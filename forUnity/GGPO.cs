...
namespace UnityGGPO {
    ...
    public static partial class GGPO {
        private const string libraryName = "libggpo";
        ...
        [DllImport(libraryName)]
        private static extern void unity_ggpo_start();
        [DllImport(libraryName)]
        private static extern void unity_ggpo_stop();
        [DllImport(libraryName)]
        private static extern int unity_ggpo_game_init(
            string playerID
            , string serverIP
            , int serverPort
            , string room
        ...
        [DllImport(libraryName)]
        public static extern int unity_ggpo_game_add_player(int player_type, int player_num, string playerID, out int phandle);
        ...
        [DllImport(libraryName)]
        private static extern int UggSetDisconnectNotifyStart(int timeout);

        [DllImport(libraryName)]
        private static extern int UggSetDisconnectTimeout(int timeout);

        [DllImport(libraryName)]
        private static extern int UggSynchronizeInput(IntPtr inputs, int length, out int disconnect_flags);

        [DllImport(libraryName)]
        private static extern int UggAddLocalInput(int local_player_handle, long input);

        [DllImport(libraryName)]
        private static extern int UggCloseSession();

        [DllImport(libraryName)]
        private static extern int UggIdle(int timeout);


        [DllImport(libraryName)]
        private static extern int UggDisconnectPlayer(int phandle);

        [DllImport(libraryName)]
        private static extern int UggSetFrameDelay(int phandle, int frame_delay);

        [DllImport(libraryName)]
        private static extern int UggAdvanceFrame();

        [DllImport(libraryName)]
        private static extern void UggLog(string text);

        [DllImport(libraryName)]
        private static extern int UggGetNetworkStats(int phandle,
        ...
        public static int SetDisconnectNotifyStart(int timeout) {
            return UggSetDisconnectNotifyStart(timeout);
        }

        public static int SetDisconnectTimeout(int timeout) {
            return UggSetDisconnectTimeout(timeout);
        }
        ...
        public static long[] SynchronizeInput(int length, out int disconnect_flags) {
            ...
            var result = UggSynchronizeInput( pnt, length, out disconnect_flags);
        ...
        public static int AddLocalInput(int local_player_handle, long input) {
            return UggAddLocalInput(local_player_handle, input);
        }

        public static int CloseSession() {
            return UggCloseSession();
        }

        public static int Idle(int timeout) {
            return UggIdle(timeout);
        }


        public static int DisconnectPlayer(int phandle) {
            return UggDisconnectPlayer(phandle);
        }

        public static int SetFrameDelay(int phandle, int frame_delay) {
            return UggSetFrameDelay(phandle, frame_delay);
        }

        public static int AdvanceFrame() {
            return UggAdvanceFrame();
        }
        ...
        public static int GetNetworkStats(int phandle,
                out int send_queue_len,
                out int recv_queue_len,
                out int ping,
                out int kbps_sent,
                out int local_frames_behind,
                out int remote_frames_behind) {
            return UggGetNetworkStats(phandle, out send_queue_len, out recv_queue_len, out ping, out kbps_sent, out local_frames_behind, out remote_frames_behind);
        }
