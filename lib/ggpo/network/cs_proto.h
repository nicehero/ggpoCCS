...
class CSProtocol : public IPollSink
{
public:
   struct Stats {
      int                 ping;
...
public:
   ...
   void Init(CS *cs, Poll &p, int queue, const std::string& selfPlayerID, const std::string& playerID, UdpMsg::connect_status *status);

   void Synchronize();
   bool GetPeerConnectStatus(int id, int *frame);
   bool IsInitialized() { return _cs != NULL; }
   ...
   void SendInputAck();
   bool HandlesMsg(const std::string &from, UdpMsg *msg);
   void OnMsg(UdpMsg *msg, int len);
   void Disconnect();
  
   void GetNetworkStats(struct GGPONetworkStats *stats);
   bool GetEvent(CSProtocol::Event &e);
...
protected:
   ...
   struct QueueEntry {
      ...
      QueueEntry() {}
      QueueEntry(int time, const std::string& playerID, UdpMsg *m) : queue_time(time), m_playerID(playerID), msg(m) { }
   };

   bool CreateSocket(int retries);
   void UpdateNetworkStats(void);
   void QueueEvent(const CSProtocol::Event &evt);
   void ClearSendQueue(void);
   void Log(const char *fmt, ...);
   void LogMsg(const char *prefix, UdpMsg *msg);
   void LogEvent(const char *prefix, const CSProtocol::Event &evt);
...
protected:
    ...
    * Event queue
    */
   RingBuffer<CSProtocol::Event, 64>  _event_queue;
};
