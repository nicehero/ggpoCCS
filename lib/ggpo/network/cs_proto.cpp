...
void
CSProtocol::Init(CS *cs,
                  Poll &poll,
                  int queue,
                  const std::string& selfPlayerID,
...
void
CSProtocol::SendInput(GameInput &input)
{
   if (_cs) {
...
bool
CSProtocol::GetEvent(CSProtocol::Event &e)
{
...
bool
CSProtocol::OnLoopPoll(void *cookie)
{
   if (!_cs) {
      return true;
   }
   ...
   case Disconnected:
      if (_shutdown_timeout < now) {
         Log("Shutting down cs connection.\n");
         _cs = NULL;
         _shutdown_timeout = 0;
      }
...
CSProtocol::SendMsg(UdpMsg *msg)
{
   ...
   _send_queue.push(QueueEntry(Platform::GetCurrentTimeMS(), m_playerID, msg));
   PumpSendQueue();
}

bool
CSProtocol::HandlesMsg(const std::string &from, UdpMsg *msg)
{
	if (!_cs) {
		return false;
	}
	return m_playerID == from;
}

void
CSProtocol::OnMsg(UdpMsg *msg, int len)
{
   bool handled = false;
   typedef bool (CSProtocol::*DispatchFn)(UdpMsg *msg, int len);
   static const DispatchFn table[] = {
      &CSProtocol::OnInvalid,             /* Invalid */
      &CSProtocol::OnSyncRequest,         /* SyncRequest */
      &CSProtocol::OnSyncReply,           /* SyncReply */
      &CSProtocol::OnInput,               /* Input */
      &CSProtocol::OnQualityReport,       /* QualityReport */
      &CSProtocol::OnQualityReply,        /* QualityReply */
      &CSProtocol::OnKeepAlive,           /* KeepAlive */
      &CSProtocol::OnInputAck,            /* InputAck */
   };
...
void
CSProtocol::UpdateNetworkStats(void)
{
...
void
CSProtocol::QueueEvent(const CSProtocol::Event &evt)
{
...
bool
CSProtocol::GetPeerConnectStatus(int id, int *frame)
{
...
void
CSProtocol::LogMsg(const char *prefix, UdpMsg *msg)
{
   switch (msg->hdr.type) {
...
void
CSProtocol::LogEvent(const char *prefix, const CSProtocol::Event &evt)
{
   switch (evt.type) {
   case CSProtocol::Event::Synchronzied:
...
bool
CSProtocol::OnInvalid(UdpMsg *msg, int len)
{
   ASSERT(false && "Invalid msg in CSProtocol");
   return false;
}

bool
CSProtocol::OnSyncRequest(UdpMsg *msg, int len)
{
...
bool
CSProtocol::OnSyncReply(UdpMsg *msg, int len)
{
   ...
   if (--_state.sync.roundtrips_remaining == 0) {
      Log("Synchronized!\n");
      QueueEvent(CSProtocol::Event(CSProtocol::Event::Synchronzied));
      _current_state = Running;
      _last_received_input.frame = -1;
      _remote_magic_number = msg->hdr.magic;
   } else {
      CSProtocol::Event evt(CSProtocol::Event::Synchronizing);
...
bool
CSProtocol::OnInput(UdpMsg *msg, int len)
{
   /*
   ...
   if (msg->u.input.num_bits) {
      ...
      while (offset < numBits) {
         ...
         if (useInputs) {
             ...
             */
            CSProtocol::Event evt(CSProtocol::Event::Input);
...
bool
CSProtocol::OnInputAck(UdpMsg *msg, int len)
{
   /*
    * Get rid of our buffered input
    */
...
bool
CSProtocol::OnQualityReport(UdpMsg *msg, int len)
{
...
bool
CSProtocol::OnQualityReply(UdpMsg *msg, int len)
{
...
bool
CSProtocol::OnKeepAlive(UdpMsg *msg, int len)
{
   return true;
}

void
CSProtocol::GetNetworkStats(struct GGPONetworkStats *s)
{
...
void
CSProtocol::SetLocalFrameNumber(int localFrame)
{
   /*
...
int
CSProtocol::RecommendFrameDelay()
{
...
void
CSProtocol::SetDisconnectTimeout(int timeout)
{
   _disconnect_timeout = timeout;
}

void
CSProtocol::SetDisconnectNotifyStart(int timeout)
{
   _disconnect_notify_start = timeout;
}

void
CSProtocol::PumpSendQueue()
{
	while (!_send_queue.empty()) {
		...
		if (_oop_percent && !_oo_packet.msg && ((rand() % 100) < _oop_percent)) {
			...
			_oo_packet.m_playerID = entry.m_playerID;
		} 
		else {

			_cs->SendTo(entry.msg, entry.msg->PacketSize(), 0,m_selfPlayerID,entry.m_playerID, 0);
	...
	if (_oo_packet.msg && _oo_packet.send_time < Platform::GetCurrentTimeMS()) {
		_cs->SendTo(_oo_packet.msg, _oo_packet.msg->PacketSize(), 0,m_selfPlayerID,_oo_packet.m_playerID, 0);
