...
CSBackend::CSBackend(GGPOSessionCallbacks *cb
							,const char *gamename
							,const char* room
    ...
    _next_spectator_frame(0)
{
    ...
    */
   _cs.InitCS(ip,serverPort,room, &_poll, this);

   _endpoints = new CSProtocol[_num_players];
...
void
CSBackend::AddRemotePlayer(const std::string& playerID,
                                  int queue)
{
   /*
   ...
   _endpoints[queue].Init(&_cs, _poll, queue,m_playerID, playerID, _local_connect_status);
   _endpoints[queue].SetDisconnectTimeout(_disconnect_timeout);
   _endpoints[queue].SetDisconnectNotifyStart(_disconnect_notify_start);
   _endpoints[queue].Synchronize();
}

GGPOErrorCode CSBackend::AddSpectator(const std::string& playerID)
{
   ...
   _spectators[queue].Init(&_cs, _poll, queue + 1000,m_playerID, playerID, _local_connect_status);
...
GGPOErrorCode
CSBackend::DoPoll(int timeout)
{
   if (!_sync.InRollback()) {
      _poll.Pump(0);

      PollCSProtocolEvents();
...
int CSBackend::Poll2Players(int current_frame)
{
   int i;
...
int CSBackend::PollNPlayers(int current_frame)
{
...
GGPOErrorCode
CSBackend::AddPlayer(GGPOPlayer *player,
                            GGPOPlayerHandle *handle)
{
   if (player->type == GGPO_PLAYERTYPE_SPECTATOR) {
      return AddSpectator(player->u.csremote.playerID);
   }
   ...
   *handle = QueueToPlayerHandle(queue);
   ...
   if (player->type == GGPO_PLAYERTYPE_REMOTE) {
      AddRemotePlayer(player->u.csremote.playerID, queue);
   }
   return GGPO_OK;
}

GGPOErrorCode
CSBackend::AddLocalInput(GGPOPlayerHandle player,
                                void *values,
                                int size)
{
   int queue;
...
GGPOErrorCode
CSBackend::SyncInput(void *values,
                            int size,
...
void
CSBackend::PollCSProtocolEvents(void)
{
   CSProtocol::Event evt;
   for (int i = 0; i < _num_players; i++) {
      while (_endpoints[i].GetEvent(evt)) {
         OnCSProtocolPeerEvent(evt, i);
      }
   }
   for (int i = 0; i < _num_spectators; i++) {
      while (_spectators[i].GetEvent(evt)) {
         OnCSProtocolSpectatorEvent(evt, i);
      }
   }
}

void
CSBackend::OnCSProtocolPeerEvent(CSProtocol::Event &evt, int queue)
{
   OnCSProtocolEvent(evt, QueueToPlayerHandle(queue));
   switch (evt.type) {
      case CSProtocol::Event::Input:
         if (!_local_connect_status[queue].disconnected) {
   ...
   case CSProtocol::Event::Disconnected:
...
void
CSBackend::OnCSProtocolSpectatorEvent(CSProtocol::Event &evt, int queue)
{
   GGPOPlayerHandle handle = QueueToSpectatorHandle(queue);
   OnCSProtocolEvent(evt, handle);
...
void
CSBackend::OnCSProtocolEvent(CSProtocol::Event &evt, GGPOPlayerHandle handle)
{
   GGPOEvent info;

   switch (evt.type) {
   case CSProtocol::Event::Connected:
      info.code = GGPO_EVENTCODE_CONNECTED_TO_PEER;
      info.u.connected.player = handle;
      _callbacks.on_event(&info);
      break;
   case CSProtocol::Event::Synchronizing:
   ...
   case CSProtocol::Event::NetworkInterrupted:
   ...
   case CSProtocol::Event::NetworkResumed:
 ...
 */
GGPOErrorCode
CSBackend::DisconnectPlayer(GGPOPlayerHandle player)
{
   int queue;
   GGPOErrorCode result;
...
void
CSBackend::DisconnectPlayerQueue(int queue, int syncto)
{
   GGPOEvent info;
...
GGPOErrorCode
CSBackend::GetNetworkStats(GGPONetworkStats *stats, GGPOPlayerHandle player)
{
   int queue;
   GGPOErrorCode result;
...
GGPOErrorCode
CSBackend::SetFrameDelay(GGPOPlayerHandle player, int delay) 
{ 
   int queue;
   GGPOErrorCode result;
...
GGPOErrorCode
CSBackend::SetDisconnectTimeout(int timeout)
{
...
GGPOErrorCode
CSBackend::SetDisconnectNotifyStart(int timeout)
{
...
GGPOErrorCode
CSBackend::PlayerHandleToQueue(GGPOPlayerHandle player, int *queue)
{
...
void
CSBackend::OnMsg(const std::string& from, UdpMsg *msg, int len)
{
   for (int i = 0; i < _num_players; i++) {
